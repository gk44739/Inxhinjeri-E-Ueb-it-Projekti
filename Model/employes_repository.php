...
function createEmploye(Employes $employe){
    ...
    if ($result == 0) {
        $query = "INSERT INTO `employes` (`photo`,`name`, `surname`, `work_position`) VALUES ('".$employe->getPhoto()."', '".$employe->getName()."', '".$employe->getSurname()."', '".$about->getWork_Position()."')";
...
function delete(Employes $employe){
    global $connection;
    $check_employe = mysqli_query($connection, "SELECT id FROM employes WHERE name='".$about->getEmri()."'");
    $result = mysqli_num_rows($check_employe);
    if($result == 1){
